...
"""
@Source: DN4 2023 Version https://github.com/WenbinLee/DN4/tree/master/
@Author: Wenbin Li (https://github.com/WenbinLee)
"""
##############################################################################
...
class ResNet_84(nn.Module):
    ...
    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, flatten=False, drop_rate=0.0,
        ...
        self.layer3 = self._make_layer(block, n_blocks[2], 320,
                                       stride=2, drop_rate=drop_rate, No_pool=No_pool, drop_block=True,
    ...
    def _make_layer(self, block, n_block, planes, stride=1, drop_rate=0.0, No_pool=False, drop_block=False,
                    block_size=1):
        downsample = None
        ...
        layers = []
        if n_block == 1:
            layer = block(self.inplanes, planes, stride, downsample, drop_rate, drop_block, block_size, No_pool,
                          self.use_se)
        else:
